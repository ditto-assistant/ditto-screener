...
Legitimate runtime lexical/canary lookup, user-scoped retrieval, answer-slot
logic, seed-subject construction, and prompt-injection defenses could therefore
look like shortcuts. Large official seed fixtures and model binaries had no
provenance marker and could look like suspicious static tables. Finally, all
medium/high findings collapsed to one generic reason, obscuring whether the
risk was source safety, private-challenge leakage, or originality.

## Updated boundary

- The prompt names allowed public-benchmark mechanisms and requires evidence of
  hidden/private values, unsafe data access, or bypass behavior before raising
  those categories.
