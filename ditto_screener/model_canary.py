    ...
    async def _handle(
        ...
        try:
            ...
            if headers.get("expect", "").casefold() == "100-continue":
                ...
                await writer.drain()
            # The canary validates that the harness consumes a hidden model
            # response, not the exact OpenAI request encoding. Some compatible
            # clients stream chunked bodies differently across architectures.
            ...
            try:
                ...
                print(
                    f"canary ignored request-body framing error: {error}",
                    flush=True,
                )
            ...
            print(f"canary rejected malformed request: {error}", flush=True)
