...
from ditto_screener.model_canary import ModelCallCanary
...
async def test_smoke_env_injected_into_run(
    ...
    user_index = sidecar_args.index("--user")
    assert sidecar_args[user_index + 1] == f"{os.getuid()}:{os.getgid()}"
...
async def test_sha_mismatch_fails_before_build(
    ...
    def _mkstemp(*_: Any, **__: Any) -> tuple[int, str]:
